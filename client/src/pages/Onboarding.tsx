    ...
    const handleNext = async () => {
        if(step === 1) {
            if(!formData.age || Number(formData.age) < 13 || Number(formData.age) > 120) {
                return toast("Age is required between 13-120.");
            }
        }
        if(step < totalSteps) {
            setStep(step + 1);
        ...
        <Toaster />
            <div className="onboarding-container">
                ...
                <div className="p-6 pt-12 onboarding-wrapper">
                    ...
                    <p className="text-slate-500 dark:text-slate-400 mt-4">Let's personalize your experiences!</p>
                </div>
                {/* Progress Indicator */}
                <div className="px-6 mb-8 onboarding-wrapper">
                    <div className="flex gap-2 max-w-2xl">
                        {[1,2,3].map((s) => (
                            <div key={s} className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${s <=step  ? "bg-emerald-500" : "bg-gray-300 dark:bg-slate-800"}`}/>
                        ))}
                    </div>
                            ...
                            <div className="flex items-center gap-4 mb-8">
                                ...
                                <div>
                                    ...
                                    <p className="text-slate-500 dark:text-slate-400 text-sm">
                                        This will help us to calculate your needs!
                                    </p>
                                </div>
                            </div>
                            <Input label="Age" type="number" value={formData.age} onChange={(v) => updateField("age", v)} placeholder="Enter your age" min={13} max={120} required/>
                        </div>
                     ) }
                    ...
                    {step === 2 && (
                        <div className="space-y-6 onboarding-wrapper">
                            ...
                            <div className="flex flex-col gap-4 max-w-2xl">
                                <Input label="Weight (kg)" type="number"  value={formData.weight} onChange={(v) => updateField("weight", v)} placeholder="Enter your weight" min={13} max={120} required/>
                        ...
                        <div className="space-y-6 onboarding-wrapper">
                            <div className="flex items-center gap-4 mb-8">
                                ...
                                <div>
                                    ...
                                    <p className="text-slate-500 dark:text-slate-400 text-sm">
                                        We will tailor your experience based on your goal!
                                    </p>
                                </div>
                            </div>
  ...
  <div className="flex gap-3 lg:justify-end">
    ...
    {step > 1 && (
      <button
        ...
        className="max-lg:flex-1 lg:px-10 px-6 py-3 bg-slate-800 hover:bg-slate-700 text-white rounded-xl font-medium transition"
      >
    ...
    <button
      onClick={handleNext}
      className="max-lg:flex-1 lg:px-10 px-6 py-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl font-medium transition"
    >
      <span className="flex items-center justify-center gap-2">
